...
var Domino_Partida = function(parent, gameData) {    
    this.parent = parent;
    this.gameData = gameData || window.gameData || {};
    ...
    this.CrearFichas = function() {
        ...
        for (var i = this.Ficha.length - 1; i > 0; i--) {
            ...
            var temp = this.Ficha[i];
            this.Ficha[i] = this.Ficha[j];
            this.Ficha[j] = temp;
        }
        // Repartir 7 fichas a cada jugador
        this.Manos = [[], [], [], []];
        for (var jugador = 0; jugador < 4; jugador++) {
            for (var k = 0; k < 7; k++) {
                this.Manos[jugador].push(jugador * 7 + k);
            }
        }
        // Solo añadir al escenario las fichas de la mano del jugador 1
        var Pos = [ -4.5, -5.0 ];
        for (var idx = 0; idx < this.Manos[0].length; idx++) {
            var fichaIdx = this.Manos[0][idx];
            Domino.Escena.add(this.Ficha[fichaIdx].Ficha);
            this.Ficha[fichaIdx].Ficha.position.set(Pos[0], 0.0, Pos[1]);
            this.Ficha[fichaIdx].RotarV();
            Pos[0] += 1.5;
            if (Pos[0] > 5.0) {
                Pos[0] = -4.5;
                Pos[1] += 2.5;
        ...
        // Las fichas de los otros jugadores quedan ocultas hasta que se jueguen
    };    
    ...
    this.Empezar = function() {        
        // Si hay nombre de usuario en gameData, asignarlo al jugador 1
        if (this.gameData && this.gameData.userName) {
            this.Opciones.NombreJugador[0] = this.gameData.userName;
            var inputNombre1 = document.getElementById('NNombre1');
            if (inputNombre1) inputNombre1.value = this.gameData.userName;
        }
        // Nombres por defecto para los demás jugadores
        for (let i = 1; i < 4; i++) {
            if (!this.Opciones.NombreJugador[i]) {
                this.Opciones.NombreJugador[i] = 'Jugador ' + (i + 1);
                var input = document.getElementById('NNombre' + (i + 1));
                if (input) input.value = this.Opciones.NombreJugador[i];
            }
        }
        // Nombres por defecto para los equipos
        for (let i = 0; i < 2; i++) {
            if (!this.Opciones.NombreEquipo[i]) {
                this.Opciones.NombreEquipo[i] = 'Equipo ' + (i + 1);
                var input = document.getElementById('NEquipo' + (i + 1));
                if (input) input.value = this.Opciones.NombreEquipo[i];
            }
        }
        this.Mano = 0;
    ...
    this.Continuar = function() {
        ...
        if (this.PuntosEquipo1 >= this.Opciones.PuntosPorPartida || this.PuntosEquipo2 >= this.Opciones.PuntosPorPartida) {
            ...
            try {
                ...
                // Determinar si el usuario actual es del equipo ganador (ajusta esta lógica según tu asignación de jugadores)
                let isWinner = false;
                if (Equipo === "1" && (gameData.userId === 1 || gameData.userId === 3)) isWinner = true;
                if (Equipo === "2" && (gameData.userId === 2 || gameData.userId === 4)) isWinner = true;
                reportGameResult(isWinner, {
                    puntosEquipo1: this.PuntosEquipo1,
                    puntosEquipo2: this.PuntosEquipo2,
                    equipoGanador: Equipo
                });
            } catch(e) {
                console.error('Error integrando con la app principal:', e);
            }
            // --- FIN INTEGRACIÓN ---
            return;
        }
...
function reportGameResult(isWinner, extraData = {}) {
  const gameData = JSON.parse(sessionStorage.getItem('gameData') || '{}');
  const totalPot = gameData.betAmount * (gameData.players ? gameData.players.length : 1);
  const houseFee = totalPot * 0.20;
  ...
  .then(data => {
    if (data.success) {
      alert(isWinner ? `¡FELICIDADES! Has ganado ${winnerAmount}€` : 'Mejor suerte la próxima vez');
