...
/* 
    TODO :
        ...
        - Puc posar ficha al acabar la m� � no estic segur si es nomes en el meu torn o siempre... xd) no influeix en la puntuaci del equip (per que es calcula abans) pero es un bug curiós
        ...
        - Nivell de dificultat (facil rand / normal)
            - Afegir predilecció per tirar una doble si es posible antes de tirar la que major puntuació tingui?
        ...
        - Idiomes (Catalá, Castellano, English)
            - El tema de las traducciones el veig complicado (sobretot por los spans que han de mostrar el nom del equipo en medio de una frase)
            - Lo mejor seria crear un HTML por cada idioma??
        V Revisar tema movil, especialmente el touch, y ver que todos los menus no sobresurran de la pantalla
            V Touch revisat, ahora parece que funciona simulando desde el chrome.
        V Tinc 2 puntuacions per partida... en el UI y en Partida.Opciones.... i hi ha lio (si el poso a 100 i recarrego la pagina, mostra el 100, pero realment conta hasta 300)
        V Entre el momento que hi ha l'animació al colocar la ficha es pot posar una ficha com si no s'haques colocat la que s'esta animant
...
// --- INICIO AUTOMÁTICO DESDE IFRAME ---
window.addEventListener('message', function(event) {
  if (event.data && event.data.type === 'GAME_INIT') {
    const gameData = event.data.data;
    window.gameData = gameData;
    if (window.Domino && typeof window.Domino_Partida === 'function') {
      window.Domino.Partida = new window.Domino_Partida(window.Domino, gameData);
      if (window.Domino.Partida && typeof window.Domino.Partida.Empezar === 'function') {
